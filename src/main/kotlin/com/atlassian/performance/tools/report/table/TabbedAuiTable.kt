...
internal class AuiTabbedTableFactory(
    private val namedTables: List<Pair<AuiTable, String>>
) {
    fun create() = TabbedAuiTable(
        tabs = namedTables
            .mapIndexed { i, (table, name) -> AuiTableTab(name, i.toString(), table) }
    )
}
...
internal class AuiTableFactory(
    ...
    private val headers = headers.mapIndexed { i, h -> AuiTableHeader(i.toString(), h) }
